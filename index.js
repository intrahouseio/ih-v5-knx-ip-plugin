...
(async () => {
  ...
  try {
    ...
    plugin.log('Plugin is start.', 1);

    plugin.params.data = await plugin.params.get();
    plugin.log('Recieve plugin parametrs: '+ util.inspect(plugin.params.data), 1);

    plugin.channels.data = await plugin.channels.get();
    plugin.log('Recieve plugin channels: '+ util.inspect(plugin.channels.data), 1);
