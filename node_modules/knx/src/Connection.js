...
FSM.prototype.prepareDatagram = function(svcType) {
  const datagram = {
    ...
    //
  AddHPAI.call(this, datagram);
  //
  switch (svcType) {
    case KnxConstants.SERVICE_TYPE.CONNECT_REQUEST:
      AddTunn.call(this, datagram);
...
// NOTE: this used to announce a hardcoded port (36710), which meant the
// underlying socket also had to bind to that fixed port. That prevented a
// second instance/fork of the plugin from starting, since the port was
// already in use. The socket is now bound to an OS-assigned ephemeral
// port (see IpTunnelingConnection.js), so we announce the port that was
// actually assigned to it instead of a fixed value.
function AddHPAI(datagram) {
  const port = (this && this.socket && this.socket.address)
    ? this.socket.address().port
    : 0;
  datagram.hpai = {
    protocol_type: 1, // UDP
    tunnel_endpoint: ips[0] + ":" + port
  };
}

// add the tunneling udp local endpoint.
// Same reasoning as AddHPAI above: report the real, dynamically-assigned
// socket port instead of a hardcoded one (36711).
function AddTunn(datagram) {
  const port = (this && this.socket && this.socket.address)
    ? this.socket.address().port
    : 0;
  datagram.tunn = {
